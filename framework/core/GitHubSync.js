 ...
 *   branch: 'main'
 * });
 * const { content, sha } = await github.readFile('apps/my-app/data.json');
 */

class GitHubSync {
  /**
   * Create a new GitHubSync instance
   ...
   * @param {string} [config.branch='main'] - Branch to sync with
   */
  constructor(config) {
   ...
   * @param {boolean} options.noCache - Skip ETag cache (force fresh read)
   * @returns {Object} - { content, sha, etag, cached, notFound }
   */
  async readFile(path, options = {}) {
    try {
      const url = `/repos/${this.owner}/${this.repo}/contents/${path}`;
      const cached = this.etagCache.get(path);
      const etag = (!options.noCache && cached) ? cached.etag : null;
      
      const response = await this._request(url, {
        headers: etag ? { 'If-None-Match': etag } : {}
      });

      // 304 Not Modified - content hasn't changed, return cached sha
      if (response.status === 304 && cached) {
        return { 
          cached: true, 
          sha: cached.sha,  // Return the cached SHA!
      ...
      // Store ETag AND SHA for future requests
      if (response.etag || response.sha) {
        this.etagCache.set(path, {
          etag: response.etag,
          sha: response.sha,
          content: content
        });
