...
class Storage {
   ...
   * @param {string} appName - Unique app identifier
   ...
   * This returns the full record objects including id, data, synced, sha, etc.
   */
  async getAllRecords() {
  ...
  saveSharedSetting(key, value) {
    ...
    settings[key] = value;
    localStorage.setItem(Storage.SHARED_SETTINGS_KEY, JSON.stringify(settings));
  }
   ...
   */
  loadSharedSettings() {
    const settingsStr = localStorage.getItem(Storage.SHARED_SETTINGS_KEY);
    return settingsStr ? JSON.parse(settingsStr) : {};
  }
